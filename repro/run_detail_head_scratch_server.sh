...
RUN_NAME="${2:-scratch/detail_head_20e_$(date +%Y%m%d_%H%M%S)}"
...
python "$SCRIPT_DIR/summarize_detail_head_scratch.py" "$BASELINE"
python "$SCRIPT_DIR/check_detail_head.py"
mkdir -p "$OUTPUT"
(
  cd "$PROJECT_ROOT/LFMN"
  CUDA_VISIBLE_DEVICES=0 PYTHONUNBUFFERED=1 python main.py \
    --dir_data "$DATA_ROOT" \
    --model LFMNDetailHead \
    --data_train DIV2K \
    --data_test DIV2K \
