...
for f in config.txt psnr_log.pt ssim_log.pt model/model_20.pt per_image_metrics/epoch_0020.pt; do
  [[ -f "$B0_REFERENCE/$f" ]] || { echo "Missing B0 artifact: $B0_REFERENCE/$f" >&2; exit 2; }
done
for expected in 'model: LFMN' 'pre_train: ' 'data_range: 1-800/801-900' 'scale: [4]' 'patch_size: 256' 'batch_size: 4' 'seed: 1' 'epochs: 20' 'lr: 0.0002' 'scheduler: cosine' 'scheduler_t_max: 150' 'eta_min: 1e-06' 'loss: 1*L1'; do
  grep -Fxq "$expected" "$B0_REFERENCE/config.txt" || { echo "B0 protocol mismatch: $expected" >&2; exit 2; }
done
...
(
  ...
  CUDA_VISIBLE_DEVICES="${GPU:-0}" PYTHONUNBUFFERED=1 "$PYTHON_BIN" main.py \
    --dir_data "$DATA_ROOT" --model LFMNSRPRV2 --data_train DIV2K --data_test DIV2K \
    --data_range '1-800/801-900' --scale 4 --patch_size 256 --batch_size 4 --n_threads 8 --ext img \
    --epochs 20 --test_every 1000 --lr 2e-4 --scheduler cosine --scheduler_t_max 150 --eta_min 1e-6 \
    --loss '1*L1' --seed 1 --print_every 100 --save_per_image_metrics --save "$RUN_NAME/srprv2"
) 2>&1 | tee "$OUTPUT/console.log"
"$PYTHON_BIN" "$SCRIPT_DIR/summarize_n12_srprv2.py" "$OUTPUT" --baseline-reference "$B0_REFERENCE" | tee "$OUTPUT/summary.txt"
