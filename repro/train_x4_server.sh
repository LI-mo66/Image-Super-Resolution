...
if [[ "$MODEL" != "LFMN" && "$MODEL" != "LFMNFeedback" && "$MODEL" != "LFMNFreq" && "$MODEL" != "LFMNOverlap" && "$MODEL" != "LFMNRDSM" && "$MODEL" != "LFMNRDSMDirect" && "$MODEL" != "LFMNBidirectional" && "$MODEL" != "LFMNPriorUpdate" && "$MODEL" != "LFMNPriorProxy" && "$MODEL" != "LFMNStageDiff" && "$MODEL" != "LFMNCrossWindow" && "$MODEL" != "LFMNCrossAxis" && "$MODEL" != "LFMNDetailHead" && "$MODEL" != "LFMNPCSTR" && "$MODEL" != "LFMNPCSTRWide" && "$MODEL" != "LFMNMixControl" ]]; then
...
if [[ "$MODEL" == "LFMNCrossWindow" ]]; then
  args+=(--cross_window_lr_mult "$CROSS_WINDOW_LR_MULT")
fi
...
echo "Cross-window learning-rate multiplier: $CROSS_WINDOW_LR_MULT"
echo "Loss: $LOSS_SPEC"
