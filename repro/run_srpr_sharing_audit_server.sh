...
N12_RUN="${N12_RUN:-}"
OUTPUT_DIR="${OUTPUT_DIR:-$PROJECT_ROOT/experiment/all_runs/srpr_sharing_audit}"
...
"$PYTHON_BIN" "$SCRIPT_DIR/check_srpr_sharing_audit.py" \
  | tee "$OUTPUT_DIR/structure_check.json"

"$PYTHON_BIN" "$SCRIPT_DIR/audit_srpr_sharing.py" \
  --checkpoint "$CHECKPOINT" \
  ...
  --confirm-images 100 \
  --workers "${WORKERS:-4}" \
  --output "$OUTPUT_DIR/srpr_sharing_audit.json" \
  | tee "$OUTPUT_DIR/console.log"

cat "$OUTPUT_DIR/srpr_sharing_audit.txt"
