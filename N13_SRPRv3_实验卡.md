...
| PropMambaSR, IEEE TMM 2026, DOI 10.1109/TMM.2026.3668650 | 跨层状态传播以减轻深层细节衰减 | PropMambaSR传播SSM隐藏状态并结合局部纹理分支和动态融合；本候选不含Mamba/SSM，只把“跨层持续状态”抽象为可直接输出的RGB重建状态 | 只能说明思想来源，不能称为PropSSM实现或改进 |
| UCAN, arXiv:2603.11680 | 轻量全局—局部建模与跨层共享 | UCAN公开方案使用窗口空间注意力、Hedgehog Attention、蒸馏式大核模块和跨层参数共享；本候选保留LFMN的TAB/LRSA，仅在重建/反馈头上共享参数 | Gemini早期“高秩注意力”概括不足以支撑正式相关工作表述 |
| LFMN | 复用八阶段全局/局部特征主干 | 替换一次性tail为输出一致的稀疏渐进重建 | 最终新颖性需继续检索2024–2026近邻 |

拟议贡献只能称为“面向轻量LFMN主干的输出一致渐进重建候选”，在完成更全面检索和实验前不宣称首创或SOTA。

思想演化边界：Gemini最初从PropMambaSR的跨层状态传播与UCAN的高效全局—局部建模/共享机制提出组合雏形；Claude将其重构为状态路由近端重建；N11/N12逐步验证后，N13只保留“持续状态、跨阶段反馈、共享轻量参数”三项抽象原则。N13代码没有Mamba选择性扫描、PropSSM隐藏状态、Hedgehog Attention或UCAN大核蒸馏模块，因此后续论文只能写“受其启发”，不能写成两篇网络的直接融合。
